...
def _get_metric(metrics: dict, *keys):
    """
    Return the first available value from the supplied keys.
    """
    for key in keys:
...
def save_metrics(
    ...
    # ---------------------------------------------------------
    # Support both the current AI extraction format and the
    # older capitalized format.
    # ---------------------------------------------------------
    ...
    net_income = _get_metric(
        metrics,
        "net_income",
        "Net Income"
    )

    operating_income = _get_metric(
        metrics,
        "operating_income",
        "Operating Income"
    )

    cash_flow = _get_metric(
        metrics,
    ...
    total_assets = _get_metric(
        metrics,
        "total_assets",
        "Total Assets"
    )

    total_liabilities = _get_metric(
        metrics,
        "total_liabilities",
        "Total Liabilities"
    )

    risk_factors = _get_metric(
        metrics,
        "top_risk_factors",
        "risk_factors",
        "Top Risk Factors"
    ...
    params = {
        ...
        "risk_factors": _to_text(risk_factors),

        "growth_drivers": _to_text(growth_drivers),
    }
